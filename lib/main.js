...
T.renderEntities = function(data, isBullet) {
    ...
    for (var i = 0; i < dataLength; i++) {
        T.renderEntitie(data[i], isBullet)
    }
};

T.explodeTank = function(playerId) {
    T.blockRefresh = true;

    var playerInfo = _.find(T.players, {id: playerId});
    ...
    /*for (var step = 0; step < 8; step++) {
        promise = promise.then(function() {
            T.ctx.drawImage(T.textures['explosion'], step * SPRITE_STEP, 0, step * SPRITE_STEP, 40,
                x, y, T.tankSize[0], T.tankSize[1]);

            return new vow.Promise().delay(100);
        });
...
T.renderEntitie = function(data, isBullet) {
    var ctx = T.ctx;
    ...
    ctx.translate((position[0] - 1) * T.cellWidth + size[0]/2, (position[1] - 1) * T.cellWidth + size[1]/2);
